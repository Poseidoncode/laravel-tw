...
執行測試時，Laravel 會自動將 [設定環境](/docs/{'12.x'}/configuration#environment-configuration) 設定為 `testing`，因為 `phpunit.xml` 檔案中定義了環境變數。Laravel 也會自動將 session 和 cache 設定為 `array` 驅動程式，這樣在測試時就不會保留任何 session 或 cache 資料。
...
> [!NOTE]
> 測試 stubs 可以使用 [stub 發布](/docs/{'12.x'}/artisan#stub-customization) 來自訂。
