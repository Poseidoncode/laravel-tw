...
Views 將你的 controller / 應用程式邏輯與你的呈現邏輯分離，並儲存在 `resources/views` 目錄中。在使用 Laravel 時，view 樣板通常使用 [Blade 樣板語言](/docs/{'12.x'}/blade) 撰寫。一個簡單的 view 可能看起來像這樣：
...
> [!NOTE]
> 尋找更多關於如何撰寫 Blade 樣板的資訊？查看完整的 [Blade 文件](/docs/{'12.x'}/blade) 開始使用。

<a name="writing-views-in-react-or-vue"></a>
### 使用 React / Vue 撰寫 Views
...
我們的 [React 和 Vue 應用程式入門套件](/docs/{'12.x'}/starter-kits) 為你的下一個由 Inertia 驅動的 Laravel 應用程式提供了一個很好的起點。
...
`.blade.php` 副檔名告訴框架該檔案包含一個 [Blade 樣板](/docs/{'12.x'}/blade)。Blade 樣板包含 HTML 以及 Blade 指令，讓你可以輕鬆地輸出值、建立 "if" 陳述式、迭代資料等等。
...
如你所見，傳遞給 `view` 輔助函式的第一個參數對應於 `resources/views` 目錄中 view 檔案的名稱。第二個參數是一個應該提供給 view 的資料陣列。在這個例子中，我們傳遞了 `name` 變數，它在 view 中使用 [Blade 語法](/docs/{'12.x'}/blade) 顯示。
...
通常，view composers 會在應用程式的其中一個 [service providers](/docs/{'12.x'}/providers) 中註冊。在這個範例中，我們假設 `App\Providers\AppServiceProvider` 將容納這個邏輯。
...
如你所見，所有 view composers 都透過 [service container](/docs/{'12.x'}/container) 解析，因此你可以在 composer 的建構式中型別提示任何你需要的依賴項。

<a name="attaching-a-composer-to-multiple-views"></a>
#### 將 Composer 附加到多個 Views
