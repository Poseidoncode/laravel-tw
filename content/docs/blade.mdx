...
<span className="p-4 text-gray-500 bg-red"></span>
```
...
```blade
<x-button ::className="{ danger: isDeleting }">
    Submit
</x-button>
```
...
```blade
<button :className="{ danger: isDeleting }">
    Submit
</button>
```
...
```blade
<x-alert type="error" :message="$message" className="mt-4"/>
```
...
```blade
<x-alert type="error" :message="$message" className="mb-4"/>
```
...
```xml
<x-card className="shadow-sm">
    <x-slot:heading className="font-bold">
        Heading
    </x-slot>

    Content

    <x-slot:footer className="text-sm">
        Footer
    </x-slot>
</x-card>
```
...
<x-dynamic-component :component="$componentName" className="mt-4" />
```
...
<input
    id="email"
    type="email"
    className="@error('email') is-invalid @else is-valid @enderror"
/>
```
...
<input
    id="email"
    type="email"
    className="@error('email', 'login') is-invalid @enderror"
/>
