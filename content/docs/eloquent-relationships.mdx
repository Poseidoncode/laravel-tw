---
title: "Eloquent 關聯 (Eloquent Relationships)"
...
#### 將「多」關聯轉換為「一」關聯 (Converting "Many" Relationships to Has One
Relationships)
...
<a name="filtering-queries-via-intermediate-table-columns"></a>
### 透過中間資料表欄位過濾查詢 (Filtering Queries via Intermediate Table
Columns)
...
<a name="chaining-orwhere-clauses-after-relationships"></a>
#### 在關聯後串接 `orWhere` 子句 (Chaining `orWhere` Clauses After
Relationships)
...
#### 關聯計數與自訂 Select 語句 (Relationship Counting and Custom Select
Statements)
...
### 計算 Morph To 關聯的相關模型數量 (Counting Related Models on Morph To
Relationships)
...
<a name="constraining-eager-loading-of-morph-to-relationships"></a>
#### 限制 `morphTo` 關聯的預先載入 (Constraining Eager Loading of `morphTo`
Relationships)
...
<a name="constraining-eager-loads-with-relationship-existence"></a>
#### 使用關聯存在性限制預先載入 (Constraining Eager Loads With Relationship
Existence)
