...
- [定義關聯 (Defining Relationships)](#defining-relationships)
  - [一對一 / 擁有一個 (One to One / Has One)](#one-to-one)
  - [一對多 / 擁有多個 (One to Many / Has Many)](#one-to-many)
  - [一對多（反向）/ 屬於 (One to Many (Inverse) / Belongs To)](#one-to-many-inverse)
  - [一對多之其一 (Has One of Many)](#has-one-of-many)
  - [遠層一對一 (Has One Through)](#has-one-through)
  - [遠層一對多 (Has Many Through)](#has-many-through)
- [範圍關聯 (Scoped Relationships)](#scoped-relationships)
- [多對多關聯 (Many to Many Relationships)](#many-to-many)
  - [檢索中間資料表欄位 (Retrieving Intermediate Table Columns)](#retrieving-intermediate-table-columns)
  - [透過中間資料表欄位過濾查詢 (Filtering Queries via Intermediate Table Columns)](#filtering-queries-via-intermediate-table-columns)
  - [透過中間資料表欄位排序查詢 (Ordering Queries via Intermediate Table Columns)](#ordering-queries-via-intermediate-table-columns)
  - [定義自訂中間資料表模型 (Defining Custom Intermediate Table Models)](#defining-custom-intermediate-table-models)
- [多型關聯 (Polymorphic Relationships)](#polymorphic-relationships)
  - [一對一 (One to One)](#one-to-one-polymorphic-relations)
  - [一對多 (One to Many)](#one-to-many-polymorphic-relations)
  - [一對多之其一 (One of Many)](#one-of-many-polymorphic-relations)
  - [多對多 (Many to Many)](#many-to-many-polymorphic-relations)
  - [自訂多型類型 (Custom Polymorphic Types)](#custom-polymorphic-types)
- [動態關聯 (Dynamic Relationships)](#dynamic-relationships)
- [查詢關聯 (Querying Relations)](#querying-relations)
  - [關聯方法與動態屬性 (Relationship Methods vs. Dynamic Properties)](#relationship-methods-vs-dynamic-properties)
  - [查詢關聯存在性 (Querying Relationship Existence)](#querying-relationship-existence)
  - [查詢關聯不存在性 (Querying Relationship Absence)](#querying-relationship-absence)
  - [查詢 Morph To 關聯 (Querying Morph To Relationships)](#querying-morph-to-relationships)
- [聚合相關模型 (Aggregating Related Models)](#aggregating-related-models)
  - [計算相關模型數量 (Counting Related Models)](#counting-related-models)
  - [其他聚合函數 (Other Aggregate Functions)](#other-aggregate-functions)
  - [計算 Morph To 關聯的相關模型數量 (Counting Related Models on Morph To Relationships)](#counting-related-models-on-morph-to-relationships)
- [預先載入 (Eager Loading)](#eager-loading)
  - [限制預先載入 (Constraining Eager Loads)](#constraining-eager-loads)
  - [延遲預先載入 (Lazy Eager Loading)](#lazy-eager-loading)
  - [自動預先載入 (Automatic Eager Loading)](#automatic-eager-loading)
  - [防止延遲載入 (Preventing Lazy Loading)](#preventing-lazy-loading)
- [插入和更新相關模型 (Inserting and Updating Related Models)](#inserting-and-updating-related-models)
  - [`save` 方法 (The `save` Method)](#the-save-method)
  - [`create` 方法 (The `create` Method)](#the-create-method)
  - [屬於關聯 (Belongs To Relationships)](#updating-belongs-to-relationships)
  - [多對多關聯 (Many to Many Relationships)](#updating-many-to-many-relationships)
- [觸碰父級時間戳記 (Touching Parent Timestamps)](#touching-parent-timestamps)
...
#### 將「多」關聯轉換為「一」關聯 (Converting "Many" Relationships to Has One Relationships)
...
「遠層一對一」關聯定義了與另一個模型的一對一關聯。但是，這種關聯表示宣告模型可以通過_第三個_模型與另一個模型的一個實例相匹配。

例如，在汽車維修店應用程式中，每個 `Mechanic` 模型可能與一個 `Car` 模型相關聯，而每個 `Car` 模型可能與一個 `Owner` 模型相關聯。雖然技師和車主在資料庫中沒有直接關係，但技師可以_通過_ `Car` 模型存取車主。讓我們看看定義此關聯所需的資料表：
...
### 一對多 (多型) (One to Many (Polymorphic))
...
### 一對多之其一 (多型) (One of Many (Polymorphic))
...
### 多對多 (多型) (Many to Many (Polymorphic))
...
<a name="chaining-orwhere-clauses-after-relationships"></a>
#### 在關聯後串接 `orWhere` 子句 (Chaining `orWhere` Clauses After Relationships)
...
上面的範例將生成以下 SQL。如你所見，`or` 子句指示查詢回傳_任何_票數大於 100 的文章。查詢不再受限於特定使用者：
...
#### 關聯計數與自訂 Select 語句 (Relationship Counting and Custom Select Statements)
...
<a name="constraining-eager-loading-of-morph-to-relationships"></a>
#### 限制 `morphTo` 關聯的預先載入 (Constraining Eager Loading of `morphTo` Relationships)
...
<a name="constraining-eager-loads-with-relationship-existence"></a>
#### 使用關聯存在性限制預先載入 (Constraining Eager Loads With Relationship Existence)
