...
> [!NOTE]
> 在資料庫 seeding 期間，[Mass assignment 保護](/docs/{'12.x'}/eloquent#mass-assignment) 會自動停用。

<a name="writing-seeders"></a>
## 撰寫 Seeders

要產生一個 seeder，請執行 `make:seeder` [Artisan 命令](/docs/{'12.x'}/artisan)。框架產生的所有 seeders 都將放置在 `database/seeders` 目錄中：

```shell
php artisan make:seeder UserSeeder
```

Seeder 類別預設只包含一個方法：`run`。當執行 `db:seed` [Artisan 命令](/docs/{'12.x'}/artisan) 時，會呼叫這個方法。在 `run` 方法中，您可以用任何方式將資料插入資料庫。您可以使用 [query builder](/docs/{'12.x'}/queries) 手動插入資料，或者您可以使用 [Eloquent model factories](/docs/{'12.x'}/eloquent-factories)。
...
> [!NOTE]
> 您可以在 `run` 方法的簽章中型別提示任何您需要的依賴項。它們將透過 Laravel [service container](/docs/{'12.x'}/container) 自動解析。

<a name="using-model-factories"></a>
### 使用 Model Factories

當然，手動指定每個 model seed 的屬性是很麻煩的。相反，您可以使用 [model factories](/docs/{'12.x'}/eloquent-factories) 來方便地產生大量的資料庫記錄。首先，請查看 [model factory 文件](/docs/{'12.x'}/eloquent-factories) 來了解如何定義您的 factories。
