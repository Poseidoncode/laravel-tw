...
本地伺服器作為 Artisan 命令執行，非常適合建立本地 AI 助手整合，如 [Laravel Boost](/docs/12.x/installation#installing-laravel-boost)。使用 `local` 方法註冊本地伺服器：
...
Laravel MCP 與 Laravel 的[驗證功能](/docs/12.x/validation)無縫整合。你可以在工具的 `handle` 方法中驗證傳入的工具參數：
...
Laravel [service container](/docs/12.x/container) 用於解析所有工具。因此，你可以在建構函式中對工具可能需要的任何依賴進行型別提示。宣告的依賴將自動解析並注入到工具實例中：
...
Laravel MCP 與 Laravel 的[驗證功能](/docs/12.x/validation)無縫整合。你可以在提示的 `handle` 方法中驗證傳入的提示參數：
...
Laravel [service container](/docs/12.x/container) 用於解析所有提示。因此，你可以在建構函式中對提示可能需要的任何依賴進行型別提示。宣告的依賴將自動解析並注入到提示實例中：
...
Laravel [service container](/docs/12.x/container) 用於解析所有資源。因此，你可以在建構函式中對資源可能需要的任何依賴進行型別提示。宣告的依賴將自動解析並注入到資源實例中：
...
有兩種方法可以驗證對你的 MCP 伺服器的存取：透過 [Laravel Sanctum](/docs/12.x/sanctum) 進行簡單的基於 token 的驗證，或透過 `Authorization` HTTP 標頭傳遞的任何 token。或者，你可以使用 [Laravel Passport](/docs/12.x/passport) 透過 OAuth 進行驗證。

<a name="oauth"></a>
### OAuth 2.1

保護你的 Web MCP 伺服器最強大的方法是使用 [Laravel Passport](/docs/12.x/passport) 進行 OAuth。
...
如果你的應用程式尚未使​​用 Laravel Passport，請按照 Passport 的[安裝和部署指南](/docs/12.x/passport#installation)將 Passport 新增到你的應用程式中。在繼續之前，你應該擁有一個 `OAuthenticatable` 模型、新的驗證 guard 和 passport keys。
...
如果你的應用程式已經在使用 [Sanctum](/docs/12.x/sanctum)，那麼新增 Passport 可能會很麻煩。在這種情況下，我們建議使用不帶 Passport 的 Sanctum，直到你有明確且必要的需求使用僅支援 OAuth 的 MCP 客戶端。

<a name="sanctum"></a>
### Sanctum

如果你想使用 [Sanctum](/docs/12.x/sanctum) 保護你的 MCP 伺服器，只需將 Sanctum 的驗證 middleware 新增到你的 `routes/ai.php` 檔案中的伺服器即可。然後，確保你的 MCP 客戶端提供 `Authorization: Bearer <token>` 標頭以確保驗證成功：
...
你可以透過 `$request->user()` 方法存取目前驗證的使用者，允許你在 MCP 工具和資源中執行[授權檢查](/docs/12.x/authorization)：
