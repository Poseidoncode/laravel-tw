---
title: Mcp
description: Laravel MCP 提供 Model Context Protocol 整合
---
...
- [安裝 (Installation)](#installation)
    - [發布路由 (Publishing Routes)](#publishing-routes)
- [建立伺服器 (Creating Servers)](#creating-servers)
    - [伺服器註冊 (Server Registration)](#server-registration)
    - [Web 伺服器 (Web Servers)](#web-servers)
    - [本地伺服器 (Local Servers)](#local-servers)
- [工具 (Tools)](#tools)
    - [建立工具 (Creating Tools)](#creating-tools)
    - [工具輸入 Schemas (Tool Input Schemas)](#tool-input-schemas)
    - [驗證工具參數 (Validating Tool Arguments)](#validating-tool-arguments)
    - [工具依賴注入 (Tool Dependency Injection)](#tool-dependency-injection)
    - [工具註解 (Tool Annotations)](#tool-annotations)
    - [條件式工具註冊 (Conditional Tool Registration)](#conditional-tool-registration)
    - [工具回應 (Tool Responses)](#tool-responses)
- [提示 (Prompts)](#prompts)
    - [建立提示 (Creating Prompts)](#creating-prompts)
    - [提示參數 (Prompt Arguments)](#prompt-arguments)
    - [驗證提示參數 (Validating Prompt Arguments)](#validating-prompt-arguments)
    - [提示依賴注入 (Prompt Dependency Injection)](#prompt-dependency-injection)
    - [條件式提示註冊 (Conditional Prompt Registration)](#conditional-prompt-registration)
    - [提示回應 (Prompt Responses)](#prompt-responses)
- [資源 (Resources)](#resources)
    - [建立資源 (Creating Resources)](#creating-resources)
    - [資源 URI 和 MIME 類型 (Resource URI and MIME Type)](#resource-uri-and-mime-type)
    - [資源請求 (Resource Request)](#resource-request)
    - [資源依賴注入 (Resource Dependency Injection)](#resource-dependency-injection)
    - [條件式資源註冊 (Conditional Resource Registration)](#conditional-resource-registration)
    - [資源回應 (Resource Responses)](#resource-responses)
- [Metadata](#metadata)
- [驗證 (Authentication)](#authentication)
    - [OAuth 2.1](#oauth)
    - [Sanctum](#sanctum)
- [授權 (Authorization)](#authorization)
- [測試伺服器 (Testing Servers)](#testing-servers)
...
[Laravel MCP](https://github.com/laravel/mcp) 為 AI 客戶端提供了一種簡單優雅的方式，透過 [Model Context Protocol](https://modelcontextprotocol.io/docs/getting-started/intro) 與你的 Laravel 應用程式互動。它提供了一個表達力強、流暢的介面來定義伺服器、工具、資源和提示，從而實現與你的應用程式的 AI 驅動互動。
...
本地伺服器作為 Artisan 命令執行，非常適合建立本地 AI 助手整合，如 [Laravel Boost](/docs/installation#installing-laravel-boost)。使用 `local` 方法註冊本地伺服器：
...
一旦註冊，你通常不需要自己手動執行 `mcp:start` Artisan 命令。相反，設定你的 MCP 客戶端（AI 代理）來啟動伺服器或使用 [MCP Inspector](#mcp-inspector)。
...
預設情況下，工具的名稱和標題源自類別名稱。例如，`CurrentWeatherTool` 的名稱將為 `current-weather`，標題為 `Current Weather Tool`。你可以透過定義工具的 `$name` 和 `$title` 屬性來自訂這些值：
...
> [!NOTE]
> 描述是工具 metadata 的關鍵部分，因為它幫助 AI 模型了解何時以及如何有效地使用該工具。
...
Laravel MCP 與 Laravel 的[驗證功能](/docs/validation)無縫整合。你可以在工具的 `handle` 方法中驗證傳入的工具參數：
...
Laravel [service container](/docs/container) 用於解析所有工具。因此，你可以在建構函式中對工具可能需要的任何依賴進行型別提示。宣告的依賴將自動解析並注入到工具實例中：
...
你可以使用 [annotations](https://modelcontextprotocol.io/specification/2025-06-18/schema#toolannotations) 增強你的工具，以向 AI 客戶端提供額外的 metadata。這些註解幫助 AI 模型了解工具的行為和功能。註解透過屬性新增到工具中：
...
對於長時間執行的操作或即時資料串流，工具可以從其 `handle` 方法回傳一個 [generator](https://www.php.net/manual/en/language.generators.overview.php)。這使得在最終回應之前可以發送中間更新給客戶端：
...
[Prompts](https://modelcontextprotocol.io/specification/2025-06-18/server/prompts) 使你的伺服器能夠分享可重複使用的提示模板，AI 客戶端可以使用這些模板與語言模型互動。它們提供了一種標準化的方式來結構化常見的查詢和互動。
...
> [!NOTE]
> 描述是提示 metadata 的關鍵部分，因為它幫助 AI 模型了解何時以及如何充分利用該提示。
...
Laravel MCP 與 Laravel 的[驗證功能](/docs/validation)無縫整合。你可以在提示的 `handle` 方法中驗證傳入的提示參數：
...
Laravel [service container](/docs/container) 用於解析所有提示。因此，你可以在建構函式中對提示可能需要的任何依賴進行型別提示。宣告的依賴將自動解析並注入到提示實例中：
...
提示可以回傳單個 `Laravel\Mcp\Response` 或 `Laravel\Mcp\Response` 實例的可迭代物件。這些回應封裝了將發送給 AI 客戶端的內容：
...
[Resources](https://modelcontextprotocol.io/specification/2025-06-18/server/resources) 使你的伺服器能夠公開 AI 客戶端可以讀取並在與語言模型互動時用作上下文的資料和內容。它們提供了一種分享靜態或動態資訊（如文件、設定或任何有助於通知 AI 回應的資料）的方式。
...
> [!NOTE]
> 描述是資源 metadata 的關鍵部分，因為它幫助 AI 模型了解何時以及如何有效地使用該資源。
...
Laravel [service container](/docs/container) 用於解析所有資源。因此，你可以在建構函式中對資源可能需要的任何依賴進行型別提示。宣告的依賴將自動解析並注入到資源實例中：
...
Laravel MCP 也支援 [MCP 規範](https://modelcontextprotocol.io/specification/2025-06-18/basic#meta)中定義的 `_meta` 欄位，這某些 MCP 客戶端或整合需要此欄位。Metadata 可以套用到所有 MCP 原語，包括工具、資源和提示，以及它們的回應。
...
有兩種方法可以驗證對你的 MCP 伺服器的存取：透過 [Laravel Sanctum](/docs/sanctum) 進行簡單的基於 token 的驗證，或透過 `Authorization` HTTP 標頭傳遞的任何 token。或者，你可以使用 [Laravel Passport](/docs/passport) 透過 OAuth 進行驗證。

<a name="oauth"></a>
### OAuth 2.1

保護你的 Web MCP 伺服器最強大的方法是使用 [Laravel Passport](/docs/passport) 進行 OAuth。
...
如果你的應用程式尚未使​​用 Laravel Passport，請按照 Passport 的[安裝和部署指南](/docs/passport#installation)將 Passport 新增到你的應用程式中。在繼續之前，你應該擁有一個 `OAuthenticatable` 模型、新的驗證 guard 和 passport keys。
...
> [!NOTE]
> 在這種情況下，我們只是使用 OAuth 作為底層可驗證模型的轉換層。我們忽略了 OAuth 的許多方面，例如 scopes。
...
如果你的應用程式已經在使用 [Sanctum](/docs/sanctum)，那麼新增 Passport 可能會很麻煩。在這種情況下，我們建議使用不帶 Passport 的 Sanctum，直到你有明確且必要的需求使用僅支援 OAuth 的 MCP 客戶端。

<a name="sanctum"></a>
### Sanctum

如果你想使用 [Sanctum](/docs/sanctum) 保護你的 MCP 伺服器，只需將 Sanctum 的驗證 middleware 新增到你的 `routes/ai.php` 檔案中的伺服器即可。然後，確保你的 MCP 客戶端提供 `Authorization: Bearer <token>` 標頭以確保驗證成功：
...
你可以透過 `$request->user()` 方法存取目前驗證的使用者，允許你在 MCP 工具和資源中執行[授權檢查](/docs/authorization)：
...
[MCP Inspector](https://modelcontextprotocol.io/docs/tools/inspector) 是一個用於測試和除錯 MCP 伺服器的互動式工具。使用它連線到你的伺服器，驗證身分，並試用工具、資源和提示。
