...
First, let's set up a channel listener using the `subscribe` method. We'll place this method call within an [Artisan command](/docs/artisan) since calling the `subscribe` method begins a long-running process:
