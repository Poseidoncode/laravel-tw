...
- [安裝 (Installation)](#installation)
    - [升級 Valet (Upgrading Valet)](#upgrading-valet)
- [提供網站服務 (Serving Sites)](#serving-sites)
    - [「Park」指令 (The "Park" Command)](#the-park-command)
    - [「Link」指令 (The "Link" Command)](#the-link-command)
    - [使用 TLS 保護網站 (Securing Sites With TLS)](#securing-sites)
    - [提供預設網站 (Serving a Default Site)](#serving-a-default-site)
    - [各網站 PHP 版本 (Per-Site PHP Versions)](#per-site-php-versions)
- [分享網站 (Sharing Sites)](#sharing-sites)
    - [在區域網路分享網站 (Sharing Sites on Your Local Network)](#sharing-sites-on-your-local-network)
- [網站專屬環境變數 (Site Specific Environment Variables)](#site-specific-environment-variables)
- [代理服務 (Proxying Services)](#proxying-services)
- [自訂 Valet 驅動 (Custom Valet Drivers)](#custom-valet-drivers)
    - [本機驅動 (Local Drivers)](#local-drivers)
- [其他 Valet 指令 (Other Valet Commands)](#other-valet-commands)
- [Valet 目錄與檔案 (Valet Directories and Files)](#valet-directories-and-files)
    - [磁碟存取 (Disk Access)](#disk-access)
...
> [!WARNING]
> `isStaticFile` 方法只有在 `serves` 方法對傳入的請求回傳 `true` 且請求 URI 不是 `/` 時才會被呼叫。
