...
description: Laravel 中介軟體提供了一種機制，用於過濾和檢查進入應用程式的 HTTP 請求。它們充當一系列層，請求在到達目標路由處理程序之前會依次經過這些層，並且還可以在回應發送後執行操作。
---
...
> [!NOTE]
> 所有 Middleware 都透過 [Service Container](/docs/container) 解析，因此您可以在 Middleware 的建構子中對所需的任何依賴項進行型別提示。
...
#### 手動管理 Laravel 的預設全域 Middleware (Manually Managing Laravels Default
Global Middleware)
...
您還可以從整個路由定義 [群組](/docs/routing#route-groups) 中排除給定的 Middleware 集：
...
<table>
  <thead>
    <tr>
      <th>`web` Middleware 群組</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>`Illuminate\Cookie\Middleware\EncryptCookies`</td>
    </tr>
    <tr>
      <td>`Illuminate\Cookie\Middleware\AddQueuedCookiesToResponse`</td>
    </tr>
    <tr>
      <td>`Illuminate\Session\Middleware\StartSession`</td>
    </tr>
    <tr>
      <td>`Illuminate\View\Middleware\ShareErrorsFromSession`</td>
    </tr>
    <tr>
      <td>`Illuminate\Foundation\Http\Middleware\ValidateCsrfToken`</td>
    </tr>
    <tr>
      <td>`Illuminate\Routing\Middleware\SubstituteBindings`</td>
    </tr>
  </tbody>
</table>
...
#### 手動管理 Laravel 的預設 Middleware 群組 (Manually Managing Laravels Default
Middleware Groups)
...
<table>
  ...
  <tbody>
    <tr>
      <td>`auth`</td>
      <td>`Illuminate\Auth\Middleware\Authenticate`</td>
    </tr>
    <tr>
      <td>`auth.basic`</td>
      <td>`Illuminate\Auth\Middleware\AuthenticateWithBasicAuth`</td>
    </tr>
    <tr>
      <td>`auth.session`</td>
      <td>`Illuminate\Session\Middleware\AuthenticateSession`</td>
    </tr>
    <tr>
      <td>`cache.headers`</td>
      <td>`Illuminate\Http\Middleware\SetCacheHeaders`</td>
    </tr>
    <tr>
      <td>`can`</td>
      <td>`Illuminate\Auth\Middleware\Authorize`</td>
    </tr>
    <tr>
      <td>`guest`</td>
      <td>`Illuminate\Auth\Middleware\RedirectIfAuthenticated`</td>
    </tr>
    <tr>
      <td>`password.confirm`</td>
      <td>`Illuminate\Auth\Middleware\RequirePassword`</td>
    </tr>
    <tr>
      <td>`precognitive`</td>
      <td>
    ...
    <tr>
      <td>`signed`</td>
      <td>`Illuminate\Routing\Middleware\ValidateSignature`</td>
    </tr>
    <tr>
      <td>`subscribed`</td>
      <td>`\Spark\Http\Middleware\VerifyBillableIsSubscribed`</td>
    </tr>
    <tr>
      <td>`throttle`</td>
      <td>
    ...
    <tr>
      <td>`verified`</td>
      <td>`Illuminate\Auth\Middleware\EnsureEmailIsVerified`</td>
    </tr>
  </tbody>
</table>
...
在 Middleware 上呼叫 `terminate` 方法時，Laravel 將從 [Service Container](/docs/container) 解析 Middleware 的新實例。如果您希望在呼叫 `handle` 和 `terminate` 方法時使用相同的 Middleware 實例，請使用 Container 的 `singleton` 方法向 Container 註冊 Middleware。通常這應該在您的 `AppServiceProvider` 的 `register` 方法中完成：
