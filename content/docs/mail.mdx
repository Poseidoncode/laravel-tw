...
輪詢傳輸會從已設定的郵件程式列表中隨機選擇一個郵件程式，然後為每封後續電子郵件切換到下一個可用的郵件程式。與有助於實現 *[高可用性](https://en.wikipedia.org/wiki/High_availability)* 的 `failover` 傳輸相反，`roundrobin` 傳輸提供 *[負載平衡](https://en.wikipedia.org/wiki/Load_balancing_(computing))*。
...
> [!WARNING]
> `$message` 變數在純文字訊息樣板中不可用，因為純文字訊息不使用內嵌附件。
...
Laravel 在傳送郵件訊息時會分派兩個事件。`MessageSending` 事件在訊息傳送之前分派，而 `MessageSent` 事件在訊息傳送之後分派。請記住，這些事件是在郵件被 *傳送* 時分派的，而不是在排入佇列時分派的。你可以在應用程式中為這些事件建立 [事件監聽器](/docs/events)：
