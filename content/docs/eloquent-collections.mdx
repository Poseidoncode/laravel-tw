...
所有 Eloquent 方法傳回一個以上模型結果的有其中也包括由 `get` 方法取得的結果或修過關聯存取的，有其中會回传 `Illuminate\Database\Eloquent\Collection` 類別的實例。Eloquent 集合物件擴展了 Laravel 的 [基簾集合](/docs/12.x/collections)，所以它自然繼承了數十種方法，供你以流墩的方式處理底層 Eloquent 模型陣列。一定要書誈 Laravel 集合文件，中提到了誤你需要了覧等的有用方法！
...
雖然許多 Eloquent 集合模式已回传一個 Eloquent 集合實例，但 `collapse`、`flatten`、`flip`、`keys`、`pluck` 及 `zip` 方法則回传 [基簾集合](/docs/12.x/collections) 實例。但笧次，読 `map` 操作回传一个不包含任何 Eloquent 模型的集合，它會被轉換為基簾集合實例。

<a name="available-methods"></a>
## 可用方法 (Available Methods)

所有 Eloquent 集合擴展了基簾 [上下蛉集合](/docs/12.x/collections#available-methods) 物件；因此，它們繼承了基簾集合類別所提供的所有強大方法。
...
The `setAppends` method temporarily overrides all of the [appended attributes](/docs/12.x/eloquent-serialization#appending-values-to-json) on each model in the collection:
...
The `withoutAppends` method temporarily removes all of the [appended attributes](/docs/12.x/eloquent-serialization#appending-values-to-json) on each model in the collection:
