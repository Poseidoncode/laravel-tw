...
- [分派 Job](#dispatching-jobs)
  ...
  - [指定最大 Job 嘗試次數 / 逾時值](#max-job-attempts-and-timeout)
...
- [處理失敗的 Job](#dealing-with-failed-jobs)
  - [清除失敗的 Job](#cleaning-up-after-failed-jobs)
...
- [測試](#testing)
  - [偽造部分 Job](#faking-a-subset-of-jobs)
...
> [!WARNING] > `redis` 佇列驅動程式不支援 `serializer` 和 `compression` Redis 選項。
...
> [!WARNING] > `WithoutOverlapping` Middleware 需要支援[鎖定](/docs/cache#atomic-locks)的快取驅動程式。目前，`memcached`、`redis`、`dynamodb`、`database`、`file` 和 `array` 快取驅動程式支援原子鎖定。
...
> [!WARNING] > `--timeout` 值應始終比 `retry_after` 設定值短至少幾秒鐘。這將確保處理凍結 Job 的 Worker 在重試 Job 之前始終被終止。如果您的 `--timeout` 選項比 `retry_after` 設定值長，您的 Job 可能會被處理兩次。
