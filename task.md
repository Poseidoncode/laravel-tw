...
- [ ] **Step 2.9**: 翻譯 `csrf.md` (CSRF 保護)
- [ ] **Step 2.10**: 翻譯 `controllers.md` (控制器)
- [ ] **Step 2.11**: 翻譯 `requests.md` (請求) & `responses.md` (回應)
- [ ] **Step 2.12**: 翻譯 `views.md` (視圖) & `blade.md` (Blade 樣板)

### Phase 3: 進階功能翻譯 (Deep Dive)

#### Batch C: 資料庫與 Eloquent (Database)
- [ ] **Step 3.1**: 翻譯 `database.md` (資料庫基礎)
- [ ] **Step 3.2**: 翻譯 `queries.md` (Query Builder)
- [ ] **Step 3.3**: 翻譯 `pagination.md` (分頁)
- [ ] **Step 3.4**: 翻譯 `migrations.md` (遷移)
- [ ] **Step 3.5**: 翻譯 `seeding.md` (Seeding)
- [ ] **Step 3.6**: 翻譯 `redis.md` (Redis)
- [ ] **Step 3.7**: 翻譯 Eloquent 相關章節 (`eloquent.md`, `eloquent-relationships.md`, `eloquent-collections.md` 等)

#### Batch D: 安全性與測試 (Security & Testing)
- [ ] **Step 4.1**: 翻譯 `authentication.md` (驗證) & `authorization.md` (授權)
- [ ] **Step 4.2**: 翻譯 `encryption.md` (加密) & `hashing.md` (雜湊)
- [ ] **Step 4.3**: 翻譯 `testing.md` (測試概論), `http-tests.md`, `console-tests.md`, `dusk.md`

### Phase 4: 其他重要組件 (Digging Deeper)
- [ ] **Step 5.1**: 翻譯 `artisan.md` (Artisan Console)
- [ ] **Step 5.2**: 翻譯 `broadcasting.md` (廣播)
- [ ] **Step 5.3**: 翻譯 `cache.md` (快取)
- [ ] **Step 5.4**: 翻譯 `collections.md` (集合 - 僅翻譯說明文字，方法名稱保留)
- [ ] **Step 5.5**: 翻譯 `events.md` (事件), `filesystem.md` (檔案儲存), `helpers.md` (輔助函式)
- [ ] **Step 5.6**: 翻譯 `mail.md` (郵件), `notifications.md` (通知), `queues.md` (佇列), `scheduling.md` (排程)

### Phase 5: 官方套件與部署 (Packages & Deployment)
- [ ] **Step 6.1**: 翻譯官方套件文件 (如 `breeze.md`, `sail.md`, `sanctum.md`, `fortify.md` 等)
- [ ] **Step 6.2**: 翻譯 `deployment.md` (部署)

## 5. 輸出驗證 (Output Verification)
翻譯完成後，請執行以下檢查：
- [ ] **格式檢查**：確認所有 Markdown 標題 (`#`), 列表 (`-`), 引用 (`>`) 格式正確且未跑版。
- [ ] **連結檢查**：確認內部連結 (Relative Links) 依然有效，且沒有錯誤地被翻譯成中文檔名（除非檔名也一併中文化，建議檔名保持英文）。
